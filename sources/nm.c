...
/*   Updated: 2022/07/20 14:57:20 by lumenthi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			get_sections(void *header, char *path, int arch, size_t size, t_section **sections)
{
	size_t shoff = 0;
	uint32_t shsize = 0;
	uint32_t shnum = 0;
	...
	while (i < shnum) {
		if (arch == 32)
			process_section32((Elf32_Shdr *)(header + shoff + i*shsize), sections);
		else
			process_section64((Elf64_Shdr *)(header + shoff + i*shsize), sections);
		i++;
	}
	return 0;
}

int		process_symbols(void *header, t_section **sections)
{
	char *strtab = NULL;
	t_section *tmp = *sections;
	while (tmp) {
		if (tmp->sh_type == 2) {// SYMTAB
			printf("FOUND STRTAB AT OFFSET: %ld\n", tmp->sh_offset);
			strtab = (char*)(header+tmp->sh_offset);
		}
		tmp = tmp->next;
	}
	printf("STRTAB: %s\n", strtab);
...
void	ft_nm(char *path, void *buffer, size_t size)
{
	...
	if (*(uint32_t*)header==0x464c457f) { // ELF Magic number
		...
		if (symtab_infos((void*)header, path, arch, size, &Sym_Shdr) == -1)
			return;
		printf("Found symbol table at offset: %lx with size: %lx", Sym_Shdr.sh_offset, Sym_Shdr.sh_size);
		// process_symbols((void*)header, &sections);
	}
}
